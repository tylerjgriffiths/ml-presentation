...
print("Fitting data with gridsearch...")

svm = SVC()
svm_cls = GridSearchCV(svm, {'kernel':('linear','rbf'), 'C':[1,10]})
...
## Get the predictions.
print("Predicting...")
svm_y_pred = svm_cls.predict(X_dev)

## Calculate and print out some metrics.
acc = accuracy_score(y_dev, svm_y_pred)
prec = precision_score(y_dev, svm_y_pred)
rec = recall_score(y_dev, svm_y_pred)
f1 = f1_score(y_dev, svm_y_pred)
