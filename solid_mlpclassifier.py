...
## Create a new MLPClassifier and fit the data.

print("Fitting data...")
print("Shape: {}".format(shape))

mlpc_cls = MLPClassifier(hidden_layer_sizes=shape)
training_start_time = datetime.datetime.now().replace(microsecond=0)
mlpc_cls.fit(X_train, y_train)
...
## Get the predictions.
print("Predicting...")
mlpc_y_pred = mlpc_cls.predict(X_dev)

## Calculate and print out some metrics.
acc = accuracy_score(y_dev, mlpc_y_pred)
prec = precision_score(y_dev, mlpc_y_pred)
rec = recall_score(y_dev, mlpc_y_pred)
f1 = f1_score(y_dev, mlpc_y_pred)
